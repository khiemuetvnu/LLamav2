...
class LLaMa:
    ...
    def _sample_top_k(self, probs, k):
        # (B, k)
        values, idxs = torch.topk(probs, k, dim = -1)
        # (B, 1)
        next_token = torch.multinomial(values, num_samples = 1)
        # (B, 1)
...
if __name__ == '__main__':
    ...
    prompts = [
        "Is Cristiano Ronaldo the best player in this world?"
    ]

    current_dir = Path(__file__).parent
    model = LLaMa.build(
        checkpoints_dir=str(current_dir / 'llama-2-7b'),
